...
public class CarController : MonoBehaviour {
	...
	void Awake () {
		rigidbody.centerOfMass = transform.FindChild("CenterOfMass").transform.localPosition;
		speedometer = new Speedometer();
		inputer = new Inputer();
		skidmarker = transform.FindChild("Skidmarker").GetComponent<SkidMarker>();
		transform.FindChild("Skidmarker").parent = null;
	}
	...
	void Update () {
		...
		audio.pitch = Mathf.Abs(engine.RPM / engine.maxRPM) + 0.25f;
	...
	//	Debug.Log (v3Velocity.magnitude);
		...
		if(steeringWheel)
			steeringWheel.localRotation = Quaternion.Lerp(steeringWheel.localRotation, Quaternion.Euler(steeringWheel.localRotation.eulerAngles.x, 0, -90 * Input.GetAxis("Horizontal")), 15 * Time.deltaTime);
		
		wheelFL.collider.steerAngle = Mathf.DeltaAngle(0, 80 * inputer.horizontal);
		wheelFR.collider.steerAngle = Mathf.DeltaAngle(0, 80 * inputer.horizontal);
